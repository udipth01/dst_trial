...
app = Flask(__name__)  # Ensure the app instance is defined here
...
def run_task(task_description):
    try:
        if "format" in task_description.lower() and "prettier" in task_description.lower():
            ...
            return "Formatted /data/format.md using Prettier", 200
        elif "count" in task_description.lower() and "wednesdays" in task_description.lower():
        ...
        elif "install" in task_description.lower() and "datagen.py" in task_description.lower():
            # Task A1: Install uv and run datagen.py with user.email
            os.system("pip install uv")
            user_email = task_description.split("with ")[1].split(" ")[0]
            os.system(f"python3 /usr/src/app/datagen.py {user_email}")
            return "Installed uv and ran datagen.py with email argument", 200
        else:
